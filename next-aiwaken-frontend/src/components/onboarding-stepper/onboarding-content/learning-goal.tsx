...
import { learningGoals } from "./mock"
...
export const LearningGoal = () => {
  ...
  const { learningGoal: selectedLearningGoal } = useUserPreferencesStore()

  function handleLearningGoalSelection(learningGoal: string) {
    setUserPreferences({ learningGoal });
    handleNext();
  }
  ...
  return (
    <div className="w-full h-full flex flex-col">
      <motion.div
        ...
        className="flex-1"
      >
        <RobotGuide message={currentStep.description || "Choose your primary objective for using AIwaken"} />
      </motion.div>
        ...
        <Card className="w-full max-w-3xl rounded-xl border border-[#9F8DFC]/20 p-6 shadow-xl md:p-8 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {learningGoals.map((goal, index) => {
              const Icon = goal.icon as LucideIcon;
              return (
                <motion.div
                  key={goal.id}
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  <Card
                    className={`p-6 cursor-pointer hover:border-[#9F8DFC] transition-colors border-2 border-[#9F8DFC40] ${selectedLearningGoal === goal.id ? "border-[#9F8DFC]" : ""
                      }`}
                    onClick={() => handleLearningGoalSelection(goal.id)}
                  >
                    <div className="flex flex-col items-center text-center relative">
                      ...
                      <span className="text-2xl font-bold text-white mb-2">{goal.title}</span>
